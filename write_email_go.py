...
path = os.path.join(
    '/Users/venessa/Projects/placement-portal-kec-admin',
    'backend/auth-service/internal/utils/email.go'
)

bt = '`'
pct = '%'

content = f'''package utils
...
<table cellpadding="0" cellspacing="0" width="100{pct}{pct}" style="padding:40px 20px">
<tr><td align="center">
<table cellpadding="0" cellspacing="0" width="600" style="max-width:600px;background:#fff;border-radius:12px">
<tr><td style="background:linear-gradient(135deg,#002147 0{pct}{pct},#003d82 100{pct}{pct});padding:40px 30px;text-align:center">
...
<table cellpadding="0" cellspacing="0" width="100{pct}{pct}" style="margin:0 0 32px">
...
<p style="margin:0;color:#002147;font-size:42px;font-weight:700;letter-spacing:8px;font-family:monospace">{pct}s</p>
</td></tr></table>
...
{bt}, otp)
	...
	if err := smtp.SendMail(addr, auth, from, []string{{toEmail}}, msg); err != nil {{
		return fmt.Errorf("failed to send email: {pct}w", err)
	}}
	return nil
}}
'''
