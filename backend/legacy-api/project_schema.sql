...
-- ==========================================
-- 5. PLACEMENT DRIVE MODULE & 6. APPLICATIONS
-- ==========================================
-- NOTE: ALL PLACEMENT DRIVE & APPLICATION TABLES (placement_drives, job_roles, drive_applications, etc) 
-- HAVE BEEN MIGRATED TO THE 'drive' SCHEMA IN THE DRIVE MICROSERVICE.
...
CREATE TABLE student_change_requests (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    student_id BIGINT REFERENCES auth.users(id) ON DELETE CASCADE,
    ...
    handled_by BIGINT REFERENCES auth.users(id),
