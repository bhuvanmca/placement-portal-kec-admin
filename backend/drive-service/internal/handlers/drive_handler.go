...
import (
	...
	"github.com/placement-portal-kec/drive-service/internal/database"
...
func NewDriveHandler(repo *repository.DriveRepository) *DriveHandler {
...
func (h *DriveHandler) CreateDrive(c *fiber.Ctx) error {
...
func (h *DriveHandler) GetDrivesForStudent(c *fiber.Ctx) error {
	repo := h.repo
	...
	drives = h.convertAttachmentsToPresigned(drives)
...
func (h *DriveHandler) GetAllDrives(c *fiber.Ctx) error {
	repo := h.repo
...
func (h *DriveHandler) UpdateDrive(c *fiber.Ctx) error {
	idStr := c.Params("id")
...
func (h *DriveHandler) PatchDriveStatus(c *fiber.Ctx) error {
...
func (h *DriveHandler) DeleteDrive(c *fiber.Ctx) error {
	id, err := strconv.ParseInt(c.Params("id"), 10, 64)
	if err != nil {
...
func (h *DriveHandler) BulkDeleteDrives(c *fiber.Ctx) error {
	var input struct {
		IDs []int64 `json:"ids"`
	}
...
func (h *DriveHandler) AdminAddApplication(c *fiber.Ctx) error {
...
func (h *DriveHandler) ExportApplicants(c *fiber.Ctx) error {
...
func (h *DriveHandler) GetApplicants(c *fiber.Ctx) error {
	driveID, err := strconv.ParseInt(c.Params("id"), 10, 64)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "Invalid Drive ID"})
	}
...
func (h *DriveHandler) EligibilityPreview(c *fiber.Ctx) error {
...
func (h *DriveHandler) ApplyForDrive(c *fiber.Ctx) error {
...
func (h *DriveHandler) OptOutDrive(c *fiber.Ctx) error {
	studentID := int64(c.Locals("user_id").(float64))
	driveID, err := strconv.ParseInt(c.Params("id"), 10, 64)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "Invalid Drive ID"})
...
// GetStudentApplications handles GET /api/v1/student/applications (Assumed based on naming)
func (h *DriveHandler) GetStudentApplications(c *fiber.Ctx) error {
	studentID := int64(c.Locals("user_id").(float64))
	repo := h.repo

	apps, err := repo.GetStudentApplications(c.Context(), studentID)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "Failed to fetch student applications: " + err.Error()})
	}

	return c.JSON(apps)
}

// AdminRemoveApplication handles POST /admin/drives/:id/remove-student
func (h *DriveHandler) AdminRemoveApplication(c *fiber.Ctx) error {
	driveID, err := strconv.ParseInt(c.Params("id"), 10, 64)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "Invalid Drive ID"})
	}
	...
	repo := h.repo
	if err := repo.WithdrawApplication(c.Context(), input.StudentID, driveID, "Removed by Admin"); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "Failed to remove application: " + err.Error()})
	}

	return c.JSON(fiber.Map{"success": true, "message": "Application removed successfully"})
}

// GetDriveByID - GET /v1/admin/drives/:id (or student equivalent if permitted)
func (h *DriveHandler) GetDriveByID(c *fiber.Ctx) error {
	idStr := c.Params("id")
	id, err := strconv.ParseInt(idStr, 10, 64)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Invalid drive ID"})
