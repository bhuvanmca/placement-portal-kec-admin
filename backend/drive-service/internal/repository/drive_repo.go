...
func (r *DriveRepository) CreateDrive(ctx context.Context, drive models.PlacementDrive) (int64, error) {
	query := `
        INSERT INTO placement_drives (
            ...
            status, excluded_student_ids, created_at
        ) VALUES (
            $1, $2, $3,
            $4, $5, $6,
            $7, $8,
...
func (r *DriveRepository) TriggerAutoStatusUpdates(ctx context.Context) {
	...
	r.DB.Exec(ctx, "UPDATE drive.placement_drives SET status = 'closed' WHERE status = 'open' AND deadline_date < NOW()")

	// 2. Complete drives safely after drive date (give a 1-day buffer)
	r.DB.Exec(ctx, "UPDATE drive.placement_drives SET status = 'completed' WHERE status NOT IN ('completed', 'cancelled', 'draft') AND drive_date + INTERVAL '1 day' < NOW()")
}
...
func (r *DriveRepository) GetEligibleDrives(ctx context.Context, studentID int64, filters map[string]interface{}) ([]models.PlacementDrive, error) {
	...
	queryStudent := `
        ...
        FROM student.student_personal sp
        LEFT JOIN public.departments dm ON sp.department = dm.code
        LEFT JOIN student.student_degrees d_ug ON sp.user_id = d_ug.user_id AND d_ug.degree_level = 'UG'
        LEFT JOIN student.student_degrees d_pg ON sp.user_id = d_pg.user_id AND d_pg.degree_level = 'PG'
        LEFT JOIN student.student_schooling sch ON sp.user_id = sch.user_id
        WHERE sp.user_id = $1
    `
	...
	for rows.Next() {
		...
		if !d.IsEligible {
			...
			if d.TwelfthPercentage != nil && *d.TwelfthPercentage > 0 && twelfthMark > 0 && twelfthMark < *d.TwelfthPercentage {
				reasons = append(reasons, fmt.Sprintf("12th marks %.1f%% below required %.1f%%", twelfthMark, *d.TwelfthPercentage))
			}
			if d.DiplomaPercentage != nil && *d.DiplomaPercentage > 0 && diplomaMark > 0 && diplomaMark < *d.DiplomaPercentage {
				reasons = append(reasons, fmt.Sprintf("Diploma marks %.1f%% below required %.1f%%", diplomaMark, *d.DiplomaPercentage))
			}
...
func (r *DriveRepository) GetEligibleDrivesCount(ctx context.Context, studentID int64, filters map[string]interface{}) (int, error) {
	...
	queryStudent := `SELECT department, batch_year FROM student.student_personal WHERE user_id = $1`
	var dept string
	var batch int
...
func (r *DriveRepository) UpdateDrive(ctx context.Context, id int64, drive *models.PlacementDrive) error {
	...
	query := `
        ...
        SET company_name=$1, job_description=$2,
            ...
            use_aggregate=$15, aggregate_percentage=$16, eligible_gender=$17,

            rounds=$18, attachments=$19,
            drive_date=$20, deadline_date=$21,
            website=$22, logo_url=$23, location=$24, location_type=$25,
            status=$26, excluded_student_ids=$27
        WHERE id = $28
    `
...
func (r *DriveRepository) GetDriveApplicants(ctx context.Context, driveID int64) ([]models.DriveApplicant, error) {
	query := `
        ...
        JOIN public.users u ON da.student_id = u.id
        JOIN student.student_personal sp ON u.id = sp.user_id
        LEFT JOIN student.student_degrees d_ug ON u.id = d_ug.user_id AND d_ug.degree_level = 'UG'
        LEFT JOIN student.student_documents sd ON u.id = sd.user_id
        WHERE da.drive_id = $1
...
func (r *DriveRepository) GetEligibleStudentsPreview(ctx context.Context, input models.PlacementDrive) ([]models.DriveApplicantDetailed, error) {
	...
	query := `
        ...
        FROM public.users u
        LEFT JOIN student.student_personal sp ON u.id = sp.user_id
        LEFT JOIN public.departments dm ON sp.department = dm.code
        LEFT JOIN student.student_schooling sch ON u.id = sch.user_id
        LEFT JOIN student.student_degrees d_ug ON u.id = d_ug.user_id AND d_ug.degree_level = 'UG'
        LEFT JOIN student.student_degrees d_pg ON u.id = d_pg.user_id AND d_pg.degree_level = 'PG'
        WHERE u.role = 'student' 
        AND u.is_active = true 
        AND (sp.placement_willingness IS NULL OR sp.placement_willingness = 'Interested')
        AND ($1::text[] IS NULL OR cardinality($1::text[]) = 0 OR sp.department = ANY($1::text[]))
...
func (r *DriveRepository) GetAdminFCMTokens(ctx context.Context) ([]string, error) {
	query := `
		SELECT fcm_token FROM public.users
...
func (r *DriveRepository) GetStudentFCMToken(ctx context.Context, studentID int64) (string, error) {
	var token string
	query := `SELECT COALESCE(fcm_token, '') FROM public.users WHERE id = $1`
...
func (r *DriveRepository) GetDriveRequests(ctx context.Context) ([]models.DriveApplicant, error) {
	query := `
        ...
        JOIN public.users u ON da.student_id = u.id
        JOIN student.student_personal sp ON u.id = sp.user_id
        LEFT JOIN public.departments d ON sp.department = d.code
        LEFT JOIN student.student_degrees d_ug ON u.id = d_ug.user_id AND d_ug.degree_level = 'UG'
        LEFT JOIN student.student_degrees d_pg ON u.id = d_pg.user_id AND d_pg.degree_level = 'PG'
        LEFT JOIN student.student_documents sd ON u.id = sd.user_id
...
func (r *DriveRepository) ApplyForDrive(ctx context.Context, studentID, driveID int64, roleIDs []int64, forceRegister bool, requestToAttend bool) (bool, string, error) {
	...
	if !forceRegister {
		...
		var isActive bool
		err := r.DB.QueryRow(ctx, "SELECT is_active FROM public.users WHERE id = $1", studentID).Scan(&isActive)
		if err != nil || !isActive {
			return false, "User is inactive or not found", nil
		}
	}
...
func (r *DriveRepository) GetStudentApplications(ctx context.Context, studentID int64) ([]models.DriveApplication, error) {
	query := `
		SELECT
			a.drive_id, a.drive_id, a.status, a.applied_at, a.updated_at, d.company_name
	...
	var apps []models.DriveApplication
	for rows.Next() {
		var a models.DriveApplication
		var companyName string
		if err := rows.Scan(&a.ID, &a.DriveID, &a.Status, &a.AppliedAt, &a.UpdatedAt, &companyName); err != nil {
			return nil, err
		}
		// A little hacky, injecting company name into Drive details for Student Applications list
		a.Drive = models.PlacementDrive{
			ID:          a.DriveID,
