...
-- Placement Drives
CREATE TABLE
IF NOT EXISTS placement_drives
(
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    posted_by BIGINT REFERENCES users
(id),
...
(status IN
('open', 'closed', 'draft', 'completed', 'cancelled')),
...
-- Job Roles per Drive
CREATE TABLE
IF NOT EXISTS job_roles
(
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    drive_id BIGINT REFERENCES placement_drives
(id) ON
DELETE CASCADE,
...
CREATE TABLE
IF NOT EXISTS drive_eligible_departments
(
    drive_id BIGINT REFERENCES placement_drives
(id) ON
DELETE CASCADE,
...
CREATE TABLE
IF NOT EXISTS drive_eligible_batches
(
    drive_id BIGINT REFERENCES placement_drives
(id) ON
DELETE CASCADE,
...
-- Drive Applications
CREATE TABLE
IF NOT EXISTS drive_applications
(
    drive_id BIGINT REFERENCES placement_drives
(id) ON
DELETE CASCADE,
...
(
    ...
    actioned_by BIGINT REFERENCES users
(id) ON
DELETE
SET NULL
,
...
WITH TIME ZONE,
    ...
    status_change_count INT DEFAULT 0,
...
CREATE TABLE
IF NOT EXISTS drive_application_roles
(
    drive_id BIGINT NOT NULL,
    student_id BIGINT NOT NULL,
    role_id BIGINT REFERENCES job_roles
(id) ON
DELETE CASCADE,
    PRIMARY KEY (drive_id, student_id, role_id)
,
    FOREIGN KEY
(drive_id, student_id) REFERENCES drive_applications
(drive_id, student_id) ON
DELETE CASCADE
);

-- Drive SPOCs (linking drives to SPOCs)
CREATE TABLE
IF NOT EXISTS drive_spocs
(
    drive_id BIGINT REFERENCES placement_drives
(id) ON
DELETE CASCADE,
