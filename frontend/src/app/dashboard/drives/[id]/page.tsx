...
export default function DriveDetailsPage({
  ...
  const fetchData = async (silent = false) => {
    try {
      ...
      const foundDrive = response.drives.find(
      ...
      if (foundDrive) {
        setDrive(foundDrive);
        try {
          const applicantsData = await driveService.getDriveApplicantsDetailed(
            parseInt(id),
          );
        ...
        try {
          ...
          const eligibleData = await driveService.eligibilityPreview(
            foundDrive as any,
          );
  ...
  const handleStatusUpdate = async (studentId: number, newStatus: string) => {
    try {
      await driveService.updateApplicationStatus(
        parseInt(id),
        studentId,
        newStatus,
      );
      toast.success(`Marked as ${newStatus}`);
      const updated = await driveService.getDriveApplicantsDetailed(
        parseInt(id),
      );
  ...
  const handleManualRegister = async (forceRoles?: number[]) => {
    ...
    const roleIds =
      forceRoles ||
      (drive && drive.roles && drive.roles.length === 1
        ? [drive.roles[0].id]
        : []);
  ...
  const openResume = async (studentId: number) => {
    try {
      const data = await driveService.getStudentDocumentUrl(
        studentId,
        "resume",
      );
      if (data && data.url) {
  ...
  const filteredApplicants = useMemo(() => {
    let filtered = (applicants || []).filter((a) => {
      ...
      const matchesStatus =
        statusFilter === "all" || a.application_status === statusFilter;
      const matchesBatch =
        filterBatch === "All" || a.batch_year?.toString() === filterBatch;
      const matchesDept = filterDept === "All" || a.department === filterDept;
      const matchesRole =
        filterRoles.length === 0 ||
        (a.applied_role_ids &&
          a.applied_role_ids.some((rid) => filterRoles.includes(rid)));
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        switch (sortBy) {
          ...
          case "register_number":
            valA = a.register_number?.toLowerCase() || "";
            valB = b.register_number?.toLowerCase() || "";
            break;
          case "department":
          ...
          case "tenth_mark":
            ...
            break;
          case "twelfth_mark":
            valA = a.twelfth_mark || 0;
            valB = b.twelfth_mark || 0;
            break;
          case "current_backlogs":
            valA = a.current_backlogs || 0;
            valB = b.current_backlogs || 0;
            break;
  ...
  const handleExport = async () => {
    try {
      ...
      const data = await driveService.exportDriveApplicants(
        parseInt(id),
        idsToExport,
      );
      ...
      link.setAttribute(
        "download",
        `${drive?.company_name}_applicants_${new Date().toLocaleDateString()}.csv`,
      );
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-gray-50/50">
      ...
      <div className="bg-white border-b px-8 py-6">
            ...
            <div className="flex items-center gap-3">
              <Link href="/dashboard/drives">
                <Button
                  variant="ghost"
                  size="icon"
                  className="-ml-3 rounded-full hover:bg-gray-100 transition-colors"
                >
              ...
              <Badge
                variant="outline"
                className="font-semibold px-2.5 py-0.5 rounded-full border-[#002147]/20 text-[#002147] bg-[#002147]/5"
              >
                {drive.drive_type}
              </Badge>
              <Badge
                ...
                <span>
                  {drive.roles && drive.roles.length > 0
                    ? drive.roles.length > 1
                      ? `${drive.roles.length} Roles`
                      : drive.roles[0].role_name
                    : "Various Roles"}
                </span>
              </div>
              <div className="flex items-center gap-1.5 transition-colors hover:text-gray-900">
            ...
            {(user?.role === "admin" || user?.role === "super_admin") && (
              <>
                <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Enter the register number of the student you want to add
                        to this drive. They will be notified via push
                        notification.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="flex items-center space-x-2 py-4">
                      <div className="grid flex-1 gap-2">
                        <Input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && handleManualRegister()
                          }
                        />
                      </div>
                    </div>
                ...
                <Button
                  variant="outline"
                  className="border-gray-200 hover:bg-gray-50 text-gray-700"
                  onClick={() => router.push(`/dashboard/drives/${id}/edit`)}
                >
                  Edit Drive
                </Button>
                ...
                {selectedIds.length > 0 && (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-52 p-2">
                      <DropdownMenuLabel className="text-xs uppercase text-gray-400 font-bold px-2 py-1.5 tracking-widest">
                        Mark Selected As
                      </DropdownMenuLabel>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                      ...
                      <DropdownMenuItem
                        onClick={() => handleBulkStatusUpdate("opted_in")}
                        className="cursor-pointer gap-2 py-2"
                      >
                        <div className="h-2 w-2 rounded-full bg-[#002147]" />{" "}
                        Opted In
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => handleBulkStatusUpdate("rejected")}
                        className="text-red-500 focus:text-red-600 cursor-pointer gap-2 py-2"
                      >
                ...
                <Button
                  className="bg-[#002147] hover:bg-[#003366] shadow-md transition-all active:scale-95"
        ...
        <Dialog open={roleDialogOpen} onOpenChange={setRoleDialogOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-[#002147]">
                Select Roles for {regNo}
              </DialogTitle>
              <DialogDescription>
                This drive has multiple roles. Please select which roles the
                student should be opted into.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
              <div className="flex items-center justify-between border-b pb-2 mb-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedManualRoles.length === drive?.roles?.length)
                      setSelectedManualRoles([]);
                    else
                      setSelectedManualRoles(
                        drive?.roles?.map((r) => r.id) || [],
                      );
                  }}
                >
                  {selectedManualRoles.length === drive?.roles?.length
                    ? "Deselect All"
                    : "Select All"}
                </Button>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-8 pl-10">
          <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
            <div className="text-xs text-purple-600/70 font-bold uppercase tracking-wider mb-1">
              Eligible Students
            </div>
          ...
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
            <div className="text-xs text-blue-600/70 font-bold uppercase tracking-wider mb-1">
              Total Applicants
            </div>
          ...
          <div className="bg-amber-50 p-4 rounded-xl border border-amber-100">
            <div className="text-xs text-amber-600/70 font-bold uppercase tracking-wider mb-1">
              Shortlisted
            </div>
            <div className="text-3xl font-black text-amber-700">
              {
                applicants.filter((a) => a.application_status === "shortlisted")
                  .length
              }
            </div>
          </div>
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
            <div className="text-xs text-emerald-600/70 font-bold uppercase tracking-wider mb-1">
              Placed
            </div>
            <div className="text-3xl font-black text-emerald-700">
              {
                applicants.filter((a) => a.application_status === "placed")
                  .length
              }
            </div>
          </div>
          <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
            <div className="text-xs text-indigo-600/70 font-bold uppercase tracking-wider mb-1">
              Opted In
            </div>
            <div className="text-3xl font-black text-indigo-700">
              {
                applicants.filter((a) => a.application_status === "opted_in")
                  .length
              }
            </div>
          </div>
      ...
      <div className="flex-1 p-8 overflow-hidden flex flex-col pt-4">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="bg-white p-2 rounded-lg border shadow-sm w-fit">
              <TabsList className="bg-transparent border-0 p-0 flex flex-wrap gap-2">
                <TabsTrigger
                  value="applicants"
                  className="data-[state=active]:bg-[#002147] data-[state=active]:text-white rounded-md transition-all px-4 sm:px-6 hover:bg-gray-100 data-[state=active]:hover:bg-[#002147]"
                >
                  Applicants
                </TabsTrigger>
                <TabsTrigger
          ...
          <TabsContent
            value="applicants"
            className="flex-1 overflow-hidden border rounded-xl bg-white shadow-sm flex flex-col"
          >
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-9 px-3 border-gray-200 hover:bg-gray-100 transition-colors"
                    >
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-48">
                    ...
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "opted_in"}
                      onCheckedChange={() => setStatusFilter("opted_in")}
                    >
                      Opted In
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "request_to_attend"}
                      onCheckedChange={() =>
                        setStatusFilter("request_to_attend")
                      }
                    >
                      Request to Attend
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                      ...
                      onCheckedChange={() => setStatusFilter("shortlisted")}
                    >
                      Shortlisted
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "placed"}
                      onCheckedChange={() => setStatusFilter("placed")}
                    >
                      Placed
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                    ...
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "opted_out"}
                      onCheckedChange={() => setStatusFilter("opted_out")}
                    >
                      Opted Out
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem
                      checked={statusFilter === "eligible"}
                      onCheckedChange={() => setStatusFilter("eligible")}
                    >
                      Eligible
                    </DropdownMenuCheckboxItem>
                  </DropdownMenuContent>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        className={`h-9 px-3 border-gray-200 hover:bg-gray-100 transition-colors ${filterRoles.length > 0 ? "border-[#002147] text-[#002147]" : ""}`}
                      >
                ...
                <TableHeader className="bg-[#f8fafc] sticky top-0 z-10 font-semibold border-b">
                  <TableRow className="hover:bg-transparent border-b">
                    {visibleColumns.profile && (
                      <TableHead
                        className="w-[300px] px-6 py-4 sticky left-0 bg-[#f8fafc] z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] cursor-pointer select-none hover:bg-gray-200 transition-colors"
                    ...
                    {visibleColumns.register_number && (
                      <SortableHeader
                    ...
                    {visibleColumns.tenth_mark && (
                      <SortableHeader column="tenth_mark" label="10th %" />
                    )}
                    {visibleColumns.twelfth_mark && (
                      <SortableHeader column="twelfth_mark" label="12th %" />
                    )}
                    {visibleColumns.diploma_mark && (
                      <SortableHeader column="diploma_mark" label="Dip %" />
                    )}
                    ...
                    {visibleColumns.history_of_backlogs && (
                      <TableHead className="font-semibold px-6 py-4 text-sm text-[#002147] bg-[#f8fafc] text-center">
                        History of Backlogs
                      </TableHead>
                    )}
                    ...
                    {visibleColumns.opt_out_reason && (
                      <TableHead className="font-semibold px-6 py-4 text-sm text-[#002147]">
                        Opt-Out Reason
                      </TableHead>
                    )}
                    ...
                    {visibleColumns.resume && (
                      <TableHead className="font-semibold px-6 py-4 text-sm text-[#002147] text-center">
                        Resume
                      </TableHead>
                    )}
                    <TableHead className="font-semibold px-6 py-4 text-sm text-[#002147] text-right">
                      Actions
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {filteredApplicants.length === 0 ? (
                    <TableRow>
                      <TableCell
                        ...
                        className="text-center h-64 text-gray-400"
                      >
                    ...
                    paginatedApplicants.map((student) => (
                      <TableRow
                        key={student.id}
                        className={`hover:bg-gray-50 border-b group transition-colors ${selectedIds.includes(student.id) ? "bg-blue-50/50" : ""}`}
                      >
                        {visibleColumns.profile && (
                          <TableCell
                            className={cn(
                              "px-6 py-4 sticky left-0 bg-white z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-inherit",
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              />
                              <Link
                                href={`/dashboard/students/${student.register_number}`}
                              ...
                              >
                                ...
                                <div className="flex flex-col">
                                  <span className="text-sm font-semibold text-gray-900 group-hover/profile:text-[#002147] transition-colors tracking-tight">
                                    {student.full_name}
                                  </span>
                        ...
                        {visibleColumns.register_number && (
                          <TableCell className="py-4 px-6 text-sm font-medium text-gray-700">
                        ...
                        {visibleColumns.department && (
                          <TableCell className="py-4 px-6">
                            <Badge
                              variant="outline"
                              className="text-xs font-bold py-0.5 px-2 h-6 border-gray-200 text-gray-600 uppercase"
                            >
                              {student.department}
                            </Badge>
                          </TableCell>
                        ...
                        {visibleColumns.tenth_mark && (
                          <TableCell className="py-4 px-6 text-sm font-medium text-gray-600">
                            {student.tenth_mark}%
                          </TableCell>
                        )}
                        {visibleColumns.twelfth_mark && (
                        ...
                        {visibleColumns.current_backlogs && (
                          <TableCell className="py-4 px-6 text-sm text-center font-semibold">
                        ...
                        {visibleColumns.application_status && (
                          <TableCell className="py-4 px-6 text-center">
                            <Badge
                              variant="outline"
                              className={
                                student.application_status === "opted_in"
                                  ? "bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 border-emerald-200"
                                  : student.application_status === "placed"
                                    ? "bg-green-50 text-green-800 ring-1 ring-inset ring-green-700/20 border-green-200"
                                    : student.application_status ===
                                        "shortlisted"
                                      ? "bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 border-amber-200"
                                      : student.application_status ===
                                          "rejected"
                                        ? "bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20 border-red-200"
                                        : student.application_status ===
                                            "opted_out"
                                          ? "bg-gray-50 text-gray-600 ring-1 ring-inset ring-gray-300/20 border-gray-200"
                                          : student.application_status ===
                                              "request_to_attend"
                                            ? "bg-[#002147]/5 text-[#002147] ring-1 ring-inset ring-[#002147]/10 border-[#002147]/20"
                                            : "bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 border-emerald-200"
                              }
                            >
                              {student.application_status
                        ...
                        {visibleColumns.opt_out_reason && (
                          <TableCell className="py-4 px-6 text-sm text-gray-500 max-w-[200px]">
                            {student.application_status === "opted_out" &&
                            student.opt_out_reason ? (
                        ...
                        {visibleColumns.roles && (
                          <TableCell className="py-4 px-6">
                            <div className="flex flex-wrap gap-1">
                              {student.applied_role_ids &&
                              student.applied_role_ids.length > 0 &&
                              drive?.roles ? (
                                student.applied_role_ids.map((roleId) => {
                                  const role = drive.roles.find(
                                    (r) => r.id === roleId,
                                  );
                                  return role ? (
                                    <Badge
                                      key={roleId}
                                      variant="secondary"
                                      className="text-[10px] whitespace-nowrap bg-blue-50 text-blue-700 hover:bg-blue-100"
                                    >
                                      {role.role_name}
                                    </Badge>
                                  ) : null;
                            ...
                            >
                              {(user?.role === "admin" ||
                                ...
                                <>
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleStatusUpdate(student.id, "opted_in")
                                    }
                                    className="cursor-pointer gap-2 py-2"
                                  >
                                    <div className="h-2 w-2 rounded-full bg-[#002147]" />{" "}
                                  ...
                                  <DropdownMenuItem
                                    ...
                                    className="cursor-pointer gap-2 py-2"
                                  >
                                    <div className="h-2 w-2 rounded-full bg-[#002147]/60" />{" "}
                                    Request to Attend
                                  </DropdownMenuItem>
                                  <DropdownMenuItem
                                    onClick={() =>
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      handleStatusUpdate(student.id, "placed")
                                    }
                                    className="cursor-pointer gap-2 py-2"
                                  >
                                    <div className="h-2 w-2 rounded-full bg-green-500" />{" "}
            ...
            <div className="flex items-center justify-between p-4 border-t bg-gray-50/50">
              ...
              <span className="text-sm text-gray-500">
                Showing{" "}
                {Math.min((page - 1) * pageSize + 1, filteredApplicants.length)}{" "}
                to {Math.min(page * pageSize, filteredApplicants.length)} of{" "}
          ...
          >
            <div className="p-3 border-b flex justify-between items-center bg-white">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-[#002147] uppercase tracking-wider">
            ...
            <div className="flex-1 overflow-auto min-h-0">
              <Table className="min-w-max">
                <TableHeader className="bg-[#f8fafc] sticky top-0 z-10 font-semibold border-b">
                  <TableRow className="hover:bg-transparent border-b">
                    <TableHead className="px-6 py-4 w-[250px] sticky left-0 bg-[#f8fafc] z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                      Profile
                    </TableHead>
                ...
                <TableBody>
                  {loadingEligible ? (
                    ...
                    paginatedEligibleStudents.map((student) => (
                      <TableRow
                        key={student.id}
                        className="hover:bg-gray-50 border-b group transition-colors"
                      >
                        <TableCell className="px-6 py-4 sticky left-0 bg-white z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-gray-50">
                          ...
                          >
                            ...
                            <div className="flex flex-col">
                              <span className="text-sm font-semibold text-gray-900 group-hover:text-[#002147] transition-colors tracking-tight">
                                {student.full_name}
                              </span>
                              <span className="text-[10px] text-gray-400 tracking-wider truncate max-w-[150px]">
                                {student.email}
                              </span>
                            </div>
                          </Link>
                        ...
                        <TableCell className="px-6 py-4">
                          <Badge
                            variant="outline"
                            className="text-xs font-bold py-0.5 px-2 text-gray-600 bg-gray-50 border-gray-200"
                          >
                            {student.department}
                          </Badge>
                        </TableCell>
                        ...
                        <TableCell className="px-6 py-4 font-bold text-[#002147] text-right">
                          {student.ug_cgpa > 0
                        ...
                        <TableCell className="px-6 py-4 font-bold text-[#002147] text-right">
                          {student.pg_cgpa > 0
                            ? student.pg_cgpa.toFixed(2)
                            : "-"}
                        </TableCell>
                        <TableCell className="px-6 py-4 text-gray-600 text-right">
                          {student.tenth_mark?.toFixed(2) || "-"}%
                        </TableCell>
                        <TableCell className="px-6 py-4 text-gray-600 text-right">
                          {student.twelfth_mark > 0
                            ? `${student.twelfth_mark.toFixed(2)}%`
                            : "-"}
                        </TableCell>
            ...
            <div className="flex items-center justify-between p-4 border-t bg-gray-50/50 mt-auto font-medium">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-600">Rows per page</span>
                  <Select
                    value={eligiblePageSize.toString()}
                    onValueChange={(val) => {
                      setEligiblePageSize(parseInt(val));
                      setEligiblePage(1);
                    }}
                  >
                    <SelectTrigger className="w-[70px] h-8">
              ...
              <span className="text-sm text-gray-500">
                Showing{" "}
                {Math.min(
                  (eligiblePage - 1) * eligiblePageSize + 1,
                  filteredEligibleStudents.length,
                )}{" "}
                to{" "}
                {Math.min(
                  eligiblePage * eligiblePageSize,
                  filteredEligibleStudents.length,
                )}{" "}
                of {filteredEligibleStudents.length} entries
              </span>
              <div className="flex gap-2">
                <Button
                  variant="outline"
          ...
          <TabsContent
            value="details"
            className="p-8 bg-white border rounded-xl h-full shadow-sm overflow-auto"
          >
            <div className="max-w-4xl space-y-10">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b-2 border-gray-100 pb-3">
                  ...
                  <h3 className="text-lg font-bold text-[#002147] uppercase tracking-wide">
                    Job Description
                  </h3>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
                  <div className="p-5 bg-white border border-gray-100 rounded-2xl shadow-sm">
                    <span className="block text-gray-400 text-[10px] font-black uppercase tracking-widest mb-2 text-muted-foreground">
                      Departments
                    </span>
                    <span className="font-bold text-gray-800 text-base">
                      {drive.eligible_departments?.join(", ") || "All"}
                    </span>
                  </div>
                  <div className="p-5 bg-white border border-gray-100 rounded-2xl shadow-sm">
                    <span className="block text-gray-400 text-[10px] font-black uppercase tracking-widest mb-2 text-muted-foreground">
                    ...
                    <span className="font-bold text-gray-800 text-base">
                      {drive.eligible_batches?.join(", ") || "All"}
                    </span>
                  </div>
                  <div className="p-5 bg-white border border-gray-100 rounded-2xl shadow-sm">
                    <span className="block text-gray-400 text-[10px] font-black uppercase tracking-widest mb-2 text-muted-foreground">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mt-2">
                  <div className="p-4 bg-blue-50/30 border border-blue-100/50 rounded-xl text-center">
                    <span className="block text-[#002147] text-[10px] font-black uppercase tracking-widest mb-1.5 opacity-60">
                      10th Mark
                    </span>
                    <span className="font-black text-[#002147] text-lg">
                  ...
                  <div className="p-4 bg-blue-50/30 border border-blue-100/50 rounded-xl text-center">
                    ...
                    <span className="font-black text-[#002147] text-lg">
                      {drive.ug_min_cgpa || drive.min_cgpa || "N/A"}
                    </span>
                  </div>
                  <div className="p-4 bg-blue-50/30 border border-blue-100/50 rounded-xl text-center">
                    <span className="block text-[#002147] text-[10px] font-black uppercase tracking-widest mb-1.5 opacity-60">
              ...
              <div className="space-y-6">
                ...
                {drive.roles && drive.roles.length > 0 ? (
                  drive.roles.map((role, idx) => (
                    <div
                      key={idx}
                      className="bg-white border rounded-xl p-6 shadow-sm mb-4 last:mb-0"
                    >
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 border-b border-gray-100 pb-4">
                        ...
                        <div className="flex gap-4">
                          <div className="px-4 py-2 bg-green-50 rounded-lg border border-green-100">
                            <div className="text-[10px] uppercase font-black text-green-700 tracking-widest mb-1">
                              CTC
                            </div>
                          ...
                          {role.salary > 0 && (
                            <div className="px-4 py-2 bg-blue-50 rounded-lg border border-blue-100">
                              <div className="text-[10px] uppercase font-black text-blue-700 tracking-widest mb-1">
                                Salary
                              </div>
              ...
              <div className="space-y-6 pb-8">
                <div className="flex items-center gap-2 border-b-2 border-gray-100 pb-3">
                  ...
                  <h3 className="text-lg font-bold text-[#002147] uppercase tracking-wide flex items-center gap-2">
                    ...
                    <Badge
                      variant="secondary"
                      className="font-black rounded-full bg-gray-100 text-gray-500 h-5 px-1.5"
                    >
                      {drive.attachments?.length || 0}
                    </Badge>
                  </h3>
                </div>
                    ...
                    {drive.attachments.map((file, idx) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-sm text-[#002147] truncate group-hover:text-blue-700 transition-colors uppercase tracking-tight">
                            {file.name}
                          </p>
          ...
          >
            <div className="flex items-center justify-between mb-8 border-b-2 border-gray-100 pb-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-bold text-[#002147] tracking-tight uppercase">
                  Drive Schedule
                </h3>
              </div>
              <Badge
                variant="outline"
                className="font-bold text-[#002147] border-[#002147]/20 px-3 py-1 bg-[#002147]/5"
              >
              ...
              {drive.rounds && drive.rounds.length > 0 ? (
                    ...
                    <div key={i} className="relative pl-10 group">
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                          <h4 className="font-bold text-xl text-[#002147] tracking-tight">
                            {round.name}
                          </h4>
                          <Badge
                            variant="secondary"
                            className="w-fit bg-blue-50 text-blue-700 border-blue-100/50 gap-1.5 font-bold"
                          >
                ...
                <div className="flex flex-col items-center justify-center p-20 border border-dashed rounded-3xl text-gray-300 space-y-4">
                  ...
                  <p className="font-bold">
                    Process timeline not yet announced
                  </p>
                </div>
              )}
            </div>
