...
export default function ChatArea({
  ...
  useEffect(() => {
    ...
    const unsubscribe = ChatService.onMessage((msg: any) => {
      ...
      if (
        ...
        (msg.group_id === groupId || Number(msg.group_id) === groupId)
      ) {
      ...
      if (
        ...
        (msg.group_id && Number(msg.group_id) === groupId)
      ) {
        setMessages((prev) => {
  ...
  useEffect(() => {
    const unsubscribe = ChatService.onTyping(
      (groupIdArg, userId, isTyping, userName) => {
  ...
  const handleSendMessage = async (
    content: string,
    type: "text" | "image" | "file" | "student_card" = "text",
    metadata = {},
  ) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white relative">
        ...
        <div className="h-16 border-b flex items-center px-6 justify-between bg-white shrink-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {groupType === "direct" &&
                otherUserId &&
                onlineUsers.has(String(otherUserId)) && (
      ...
      {!isSelectionMode && pinnedMessages.length > 0 && (
        <div
          onClick={() =>
            scrollToMessage(pinnedMessages[pinnedMessages.length - 1].id)
          }
      ...
      <ScrollArea className="flex-1 px-2 py-1 bg-gray-50/30">
        <div
          className="flex flex-col gap-1 max-w-3xl mx-auto pb-2"
          ref={scrollRef}
        >
          ...
          {!isLoading && !hasOlder && messages.length > 0 && (
            <div className="flex flex-col items-center justify-center py-6 text-center space-y-1">
              <p className="text-xs font-semibold text-gray-500">
                You are on the top of this chat
              </p>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
              ...
              <p className="text-sm text-gray-500 max-w-xs">
                Start the conversation by sending a message below or just say
                click to say Hi!
              </p>
              <button
            ...
            messages.map((msg, index) => {
              ...
              const isDeleted =
                (msg.type as string) === "deleted" || msg.metadata?.isDeleted;
              ...
              const prevDate = prevMsg
                ? new Date(prevMsg.created_at || Date.now())
                : null;
              ...
              const nextDate = nextMsg
                ? new Date(nextMsg.created_at || Date.now())
                : null;
              const isNextSameTime =
                nextDate &&
                isValid(msgDate) &&
                isValid(nextDate) &&
              ...
              const showAvatar =
                ...
                (index === 0 ||
                  ...
                  isDifferentDay);
              const showTime =
                !isNextSameSender || !isNextSameTime || isDifferentDay;
              ...
              return (
                <React.Fragment key={msg.id || index}>
                  ...
                  >
                    ...
                    <div
                      className={`${isSelectionMode ? "opacity-100 w-full" : "opacity-0 w-0 overflow-hidden"} flex justify-center transition-all duration-300`}
                    >
                      <div
                      ...
                      >
                        <div
                          className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isSelected ? "bg-primary border-primary" : "border-gray-300 bg-white"}`}
                        >
                          {isSelected && (
                    ...
                    <div
                      className="min-w-0"
                      onDoubleClick={() => {
                        if (!isDeleted) handleReply(msg);
                      }}
                    >
                      <div
                        className={`flex gap-3 group items-center w-full ${isMe ? "justify-end" : "justify-start"}`}
                      >
                        ...
                        >
                          ...
                          {isDeleted ? (
                            ...
                            <>
                              ...
                              {replyParent && (
                                <div
                                  onClick={() =>
                                    scrollToMessage(replyParent.id)
                                  }
                                  className={`mb-1 text-xs border-l-2 pl-2 py-1 cursor-pointer hover:bg-gray-100 rounded-r ${isMe ? "border-primary/50 text-primary/80" : "border-gray-300 text-gray-500"}`}
                                >
                                  <span className="font-bold">
                                    {replyParent.sender_name}
                                  </span>
                                  :{" "}
                                  {getMessagePreviewText(replyParent).substring(
                                    0,
                                    50,
                                  )}
                                  {getMessagePreviewText(replyParent).length >
                                  50
                                    ? "..."
                                    : ""}
                                </div>
                              ...
                              <SwipeToReply
                                ...
                                onToggleSelect={(e) =>
                                  toggleSelection(msg.id, e)
                                }
                              >
                                {msg.type === "student_card" ? (
                                  <div
                                    className={`p-3 rounded-lg border bg-white shadow-sm select-none hover:select-text ${isMe ? "border-primary/20" : "border-gray-200"} `}
                                  >
                                    {/* Student Card UI */}
                                    <p>Student Profile</p>
                                  </div>
                          ...
                          <span
                            className={`text-[10px] text-gray-400 mt-1 px-1 flex items-center gap-1 min-h-[16px] transition-all ${showTime ? "opacity-100" : "opacity-0 h-0 hidden"}`}
                          >
                            {isMe && msg.is_pinned && (
                              <Pin className="h-3 w-3" />
                            )}
                            {isValid(new Date(msg.created_at || Date.now()))
                              ? format(
...
function FilePreviewCard({ msg, isMe }: { msg: ChatMessage; isMe: boolean }) {
  ...
  const isAudio =
    msg.metadata?.mimeType?.startsWith("audio/") || msg.metadata?.isVoice;

  if (isImage) {
    return (
      <div
        className={`relative aspect-video w-48 bg-gray-100 rounded-lg overflow-hidden cursor-pointer border ${isMe ? "border-primary/20" : "border-gray-200"}`}
  ...
  if (isAudio) {
    return (
      <div
        className={`flex items-center gap-3 min-w-[220px] max-w-[300px] px-3 py-2.5 rounded-xl ${
          isMe
      ...
      >
        <Mic
          className={`h-4 w-4 shrink-0 ${isMe ? "text-primary-foreground/70" : "text-gray-400"}`}
        />
        <audio
          controls
          src={msg.content}
  ...
  const displayFileName =
    fileName === "Attachment"
      ? msg.content.split("/").pop()?.split("?")[0] || "Attachment"
      : fileName;
  ...
  return (
    ...
    >
      <div
        className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${isMe ? "bg-white/20" : "bg-gray-100"}`}
      >
      ...
      <div className="flex-1 overflow-hidden min-w-0 text-left">
        ...
        {fileSize && (
          <p
            className={`text-xs mt-0.5 ${isMe ? "text-primary-foreground/80" : "text-gray-500"}`}
          >
...
function SwipeToReply({
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (validDrag) {
      ...
      const resistance =
        absDiff > threshold
          ? threshold + (absDiff - threshold) * 0.15
          : absDiff;
...
function ForwardDialog({
  ...
  onForward: (targetIds: number[], type: "user" | "group") => void;
}) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (open && user?.id) {
      ChatService.getGroups(Number(user.id))
        .then(setGroups)
        .catch(console.error);
    }
  }, [open, user?.id]);
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="max-h-[300px] overflow-y-auto space-y-2 py-2">
          {groups.map((g) => (
            ...
            >
              <div
                className={`w-5 h-5 rounded-full border flex items-center justify-center ${selectedTargets.has(g.id) ? "bg-primary border-primary" : "border-gray-300"}`}
              >
                {selectedTargets.has(g.id) && (
              ...
              <Avatar className="h-10 w-10">
                <AvatarImage
                  src={
                    g.image_url ||
                    `https://api.dicebear.com/7.x/initials/svg?seed=${g.name}`
                  }
                />
                <AvatarFallback>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button onClick={handleSend} disabled={selectedTargets.size === 0}>
            Send
          </Button>
        </div>
      </DialogContent>
